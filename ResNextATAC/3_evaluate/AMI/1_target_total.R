...
mat=Matrix::readMM('pmat_target_total.mtx')
dim(mat)
library(data.table)
...
DimPlot(obj,reduction = 'tsne',label = T,pt.size = 0.05,cols = colorRampPalette(pal_npg(alpha = 0.6)(10))(35))+NoLegend()
DimPlot(obj,group.by = 'lineage',label = T,reduction = 'tsne',cols = colorRampPalette(pal_npg(alpha = 0.6)(10))(10),pt.size = 0.05)
saveRDS(obj,file = 'obj_target_total.rds')
...
write.csv(tsne,'tsne_target_total.csv')

write.csv(as.data.frame(obj@meta.data),'meta_cluster_target_total.csv')

meta=fread('Female_mus_anno_5w.txt',header = T)
meta=as.data.frame(meta)
rownames(meta)=meta$barcode
meta1=meta[colnames(obj),]
write.csv(meta1,'meta_target_total.csv')
