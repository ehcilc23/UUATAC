...
table(df$chr)
fwrite(df,file = './pred_peak_human_sub.bed',quote = F,row.names = F,col.names = F,sep = '\t')

#run in terminal
#bedClip pred_peak_hg38_sub.bed chrom.size pred_peak_hg38_sub_filter.bed
#bedtools sort -i pred_peak_hg38_sub_filter.bed >pred_peak_hg38_sub_filter_sort.bed
#bedRemoveOverlap pred_peak_hg38_sub_filter_sort.bed pred_peak_hg38_sub_filter_sort_rmdup.bed
#GENOME=/file/path/prefix/hg38/hg38.fa
#bedtools getfasta -tab -fi $GENOME -bed pred_peak_hg38_sub_filter_sort_rmdup.bed -fo Region.pred.143w.Human.input.txt